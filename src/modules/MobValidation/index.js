...
class MobValidation extends Component<Props> {
  ...
  getOtp = () => {
    ...
    if (this.mobNum.value.length === 10) {
      ...
      const apiUrl = `http://15.206.91.199:8080/send_otp?session_id=${sessionId}&phone_no=${this.mobNum.value}`
  ...
  validateOtp = () => {
    ...
    if (this.otpNum.value.length === 6) {
      const apiUrl = `http://15.206.91.199:8080/validate_otp?session_id=${sessionIdState}&phone_no=${this.mobNum.value}&otp=${this.otpNum.value}`
      fetch(apiUrl).then(res => res.json())
        .then(response => {
